...
def foodHeuristic(state, problem):
    ...
    def dsu_find_union(dsu_parent, dsu_rank, x, y):
        x_parent = dsu_find_parent(dsu_parent, x) 
        y_parent = dsu_find_parent(dsu_parent, y) 
        if(dsu_rank[x_parent]>dsu_rank[y_parent]):
            dsu_parent[y_parent]=x_parent
            dsu_rank[x_parent]+=1
        else:
            dsu_parent[x_parent]=y_parent
            dsu_rank[y_parent]+=1
    def dsu_find_parent(dsu_parent, child):
        if dsu_parent[child] == -1:
            return child
        dsu_parent[child]=dsu_find_parent(dsu_parent, dsu_parent[child])
        return dsu_parent[child]

    current_state, foodGrid = state
    foods_to_eat = foodGrid.asList()

    if(len(foods_to_eat)==0):
        return 0

    food_game_grid = []
    for i in range(len(foods_to_eat)):
        for j in range(1, len(foods_to_eat)):
            if(i!=j):
                edge_cost=util.manhattanDistance(foods_to_eat[i], foods_to_eat[j])
                food_game_grid.append([ edge_cost,foods_to_eat[i], foods_to_eat[j]])

    food_point=0
    edges= 0

    food_game_grid = sorted(food_game_grid, key=lambda x:x[0])
    ...
    while edges < len(foods_to_eat) - 1:
        distance, starting_point, ending_point = food_game_grid[food_point]
        food_point += 1
        parent_starting_point = dsu_find_parent(dsu_parent, starting_point)
        parent_ending_point = dsu_find_parent(dsu_parent, ending_point)

        if parent_starting_point != parent_ending_point:
            edges += 1
            minimum_spanning_tree += distance
            dsu_find_union(dsu_parent, dsu_rank, parent_starting_point, parent_ending_point)

    nearest_food_distance= min([util.manhattanDistance(current_state, food) for food in foods_to_eat])

    return nearest_food_distance + minimum_spanning_tree
...
class AnyFoodSearchProblem(PositionSearchProblem):
    ...
    def isGoalState(self, state):
        ...
        """
        x,y = state
        foods_to_eat=self.food.asList()
        for i in range(len(foods_to_eat)):
            if(state==foods_to_eat[i]):
                return True
        return False
